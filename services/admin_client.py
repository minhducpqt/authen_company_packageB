...
class AdminClient:
    ...
    def create_project(self, access: str, payload: dict, *, company_code: str | None = None) -> dict:
        ...
        # fallback: nếu code=200 mà không có id → thử tra lại
    ...
    def update_project(self, access: str, project_id: int, payload: dict) -> dict:
        with httpx.Client(base_url=self.base_url, timeout=20.0) as c:
            r = c.put(f"/api/v1/projects/{project_id}", json=payload, headers=_auth_headers(access))
        j = None
        try:
            j = r.json()
        except Exception:
    ...
    # --- Lots ---
    def create_lot(self, access: str, payload: dict) -> dict:
        """
        payload phải có project_id
        """
        with httpx.Client(base_url=self.base_url, timeout=20.0) as c:
            r = c.post("/api/v1/lots", json=payload, headers=_auth_headers(access))
        j = None
        try:
            j = r.json()
        except Exception:
