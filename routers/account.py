...
# --- NEW: Email config pages for SUPER_ADMIN ---
@router.get("/company/{company_code}/email-config", response_class=HTMLResponse)
async def company_email_config_form(request: Request, company_code: str):
    me = await _me(request)
    if not me:
        return RedirectResponse(url=f"/login?next=/account/company/{quote(company_code)}/email-config", status_code=303)
    role = (me.get("role") or "").upper()
    if role != "SUPER_ADMIN":
        return RedirectResponse(url="/account?err=forbidden", status_code=303)
    ...
    try:
        async with httpx.AsyncClient(base_url=SERVICE_A_BASE_URL, timeout=8.0) as client:
            headers = {"Authorization": f"Bearer {acc}"}
            # API Service A: lấy cấu hình hiện tại
            r = await client.get(f"/api/v1/admin/email/smtp-config?company_code={quote(company_code)}", headers=headers)
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "company_code": company_code, "cfg": cfg or {}, "load_err": load_err, "msg": request.query_params.get("msg")}
    )


@router.post("/company/{company_code}/email-config/save")
async def company_email_config_save(
    request: Request,
    company_code: str,
    from_email: str = Form(...),
    ...
    payload = {
        ...
        "app_password": (app_password or "").strip() or None,
    ...
    try:
        async with httpx.AsyncClient(base_url=SERVICE_A_BASE_URL, timeout=10.0) as client:
            headers = {"Authorization": f"Bearer {acc}"}
            # API Service A: lưu cấu hình (PUT idempotent)
            r = await client.put("/api/v1/admin/email/smtp-config", headers=headers, json=payload)
        if r.status_code == 200:
            return RedirectResponse(url=f"/account/company/{quote(company_code)}/email-config?msg=Đã lưu cấu hình", status_code=303)
        else:
            return RedirectResponse(url=f"/account/company/{quote(company_code)}/email-config?msg=Lưu thất bại (HTTP {r.status_code})", status_code=303)
    except Exception as e:
        return RedirectResponse(url=f"/account/company/{quote(company_code)}/email-config?msg={quote(str(e))}", status_code=303)
